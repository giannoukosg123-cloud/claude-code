...
from . import config, discovery, download, http_client, manifest as manifest_mod
...
def run_discovery(session, logger: logging.Logger) -> Tuple[List[discovery.DiscoveredEntry], str, str]:
    """Runs library.asp -> display_doc.asp -> header.asp and parses the
    full logical_position -> current_id mapping. Raises DiscoveryError with
    a diagnostic message if anything about this invariant fails -- this
    must never be silently patched over."""
    lib_resp = discovery.fetch_library(session, logger)
    log_event(
        logger, logical_position=None, current_id=None, action="FETCH_LIBRARY",
        source_url=discovery.library_url(), local_path=None, status=str(lib_resp.status_code),
    )
    if lib_resp.status_code != 200:
        raise DiscoveryError(f"library.asp returned HTTP {lib_resp.status_code}")
    if not discovery.confirm_segment_present(lib_resp.text):
        raise DiscoveryError(
            f"library.asp did not mention seg={config.SEG} anywhere -- wrong item/segment, or page layout changed."
        )

    doc_resp = discovery.fetch_display_doc(session, logger)
    log_event(
        logger, logical_position=None, current_id=None, action="FETCH_DISPLAY_DOC",
        source_url=discovery.display_doc_url(), local_path=None, status=str(doc_resp.status_code),
    )
    ...
    header_resp = discovery.fetch_header(session, header_url, logger)
    log_event(
        logger, logical_position=None, current_id=None, action="FETCH_HEADER",
    ...
    log_event(
        logger, logical_position=None, current_id=None, action="DISCOVER_HEADER_ENTRIES",
        source_url=header_url, local_path=None, status="ok",
        extra=f"strategy={strategy} count={len(entries)}",
    )

    if len(entries) != config.EXPECTED_SCANS:
        ...
        raise DiscoveryError(
            f"Expected exactly {config.EXPECTED_SCANS} entries, found {len(entries)} "
            f"(strategy={strategy}). Missing logical positions: {missing[:20]}"
...
def process_scan(
    ...
    """Download + validate one scan, update manifest_data in place, resuming
    if a valid file already exists. Returns one of "skipped_ok",
    "downloaded_ok", "error" for the caller's tallying."""
    scan_entry = manifest_mod.find_scan_entry(manifest_data, entry.logical_position)
    if scan_entry is None:
        scan_entry = manifest_mod.new_scan_entry(entry.logical_position, entry.current_id, entry.source_url)
        manifest_data["scans"].append(scan_entry)

    local_path = config.SCANS_DIR / scan_entry["local_filename"]
